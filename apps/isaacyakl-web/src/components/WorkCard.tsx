...
const WorkCard = ({ title = "", id = title, date = "", role = "", image = "", summary = "" }: { title?: string; id?: string; date?: string; role?: string; image?: string; summary?: string }) => {
    ...
    return (
        <Link href={`/work/${id}`} title={`View ${title}`} className="block text-black! hover:no-underline! col-span-2 xl:col-span-1 row-span-1 transition-all duration-500 ease-in-out saturate-80 hover:saturate-125">
            <div className="flex flex-col h-125 rounded overflow-hidden border border-neutral-300">
                <div className="relative w-full h-62.5 max-h-62.5 grow shrink-0 overflow-hidden">
