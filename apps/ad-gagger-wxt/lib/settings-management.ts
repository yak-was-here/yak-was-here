...
export function getSiteConfigurationsForURL(
    ...
    for (const siteConfiguration of settings.siteConfigurations) {
        if (new MatchPattern(siteConfiguration.matchString).includes(url)) {
...
async function retrieveSettingsFromStorage(): Promise<Settings | null> {
    ...
    if (settings !== null) {
        ...
        console.log(
            `Successfully retrieved settings from storage. Last modified: `,
            lastModifiedDate
        );
    }
...
export const validateSettings = (userSettings: Settings): Settings => {
    const validatedSettings = JSON.parse(
    ...
    const defaultSiteConfigs_UserEnabledSettings = new Map<string, boolean>(
        validatedSettings.siteConfigurations
            .filter((userSiteConfig) => {
                return defaultSiteConfigurations.find(
                    (defaultSiteConfig) =>
                        defaultSiteConfig.id === userSiteConfig.id
                );
            })
    ...
    const defaultSiteConfigs_WithUserEnabledSettings: SiteConfiguration[] =
        defaultSiteConfigurations.map((defaultSiteCfg) => {
            const clonedDefaultSiteCfg = JSON.parse(
                JSON.stringify(defaultSiteCfg)
            ) as SiteConfiguration;
            const overrideEnabled = defaultSiteConfigs_UserEnabledSettings.get(
                defaultSiteCfg.id
            );
    ...
    const userCreatedSitConfigs: SiteConfiguration[] =
        validatedSettings.siteConfigurations.filter((cfg) => {
            const isDefaultId = defaultSiteConfigurations.find(
                (c) => c.id === cfg.id
            );
    ...
    validatedSettings.siteConfigurations = [
        ...defaultSiteConfigs_WithUserEnabledSettings,
        ...userCreatedSitConfigs,
    ];
